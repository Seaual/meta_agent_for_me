...
description: |
  ...
  Includes metadata, attendees, agenda, decisions, action items (owner + due date), and follow-up steps.
  Activate when: user asks to generate meeting minutes, create meeting notes, summarize a transcript.
  Keywords: meeting minutes, 会议纪要, transcript summary, 会议记录.
  Do NOT use for: general document writing, email drafting, non-meeting content.
...
```bash
# 确保转录文件存在
ls -la .claude/workspace/transcript.txt
```
...
```
根据 transcript.txt 生成会议纪要，遵循 Strict Minutes Schema 格式。
```
...
```bash
# 检查生成的纪要文件
cat .claude/workspace/draft-minutes.md | head -50
```
